"""
Code do postrocessing of NORI images
"""
import os
import glob
import numpy as np
...
def start(data, notify):
    ...
    decomp_matrix = sio.loadmat(
        os.path.join(calibration_folder, decomp_m_filename)
    )  # M
    ...
    with open(os.path.join(calibration_folder, "decomp_data_bg.m"), encoding='utf-8') as file:
        decomp_data = file.read()
    channels = (
        decomp_data.split("channelnames = {")[1]
        .split("}\n")[0]
        .replace("'", "")
        .split(",")
    )
    ...
    cal_paths = (
        decomp_data.split("samplenamestr = {")[1]
        .split("}\n")[0]
        .replace("'", "")
        .split(",")
    )
    ...
    if calculate_z_flag:
        ...
        sio.savemat(
            os.path.join(corr_mask_path, "maxz.mat"),
            {
    ...
    for folder in folders[0:]:
        ...
        # Find all .oir files
        oir_files = glob.glob(
            os.path.join(os.path.join(path, folder), "**", OIR_EXT), recursive=True
        )
            ...
            if "Zone.Identifier" not in oir_file:
                ...
                if file_name[:3] != "Map":
                    ...
                    if "_NORI_" in oir_file.split("\\")[-2]:
                        immask_channel, renamed_file = match_channels(
                            image_np,
                            oir_file,
                            file_name,
                            masks,
        ...
        samples = find_decomp_files(all_decomp_files, file_separator)
